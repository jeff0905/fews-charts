            ...
            <v-flex  xs6 sm6 md6 elevation-1 style="min-height:240px;">
              <v-layout row>
                <v-flex xs12 sm12 md6 xl6 lg6>
                  <Charts :option="barOption" :theme="'light'" :render="'svg'" class="chart"/>
                </v-flex>
                <v-flex xs12 sm12 md6 xl6 lg6>
                  <Charts :option="barOption2" :theme="'light'" :render="'svg'" class="chart"/>
                </v-flex>
              </v-layout>
            </v-flex>
            <v-flex xs6 sm6 md6 elevation-1 style="min-height:240px;">
              <Charts :option="barOption2" class="chart"/>
            </v-flex>
          </v-layout>
        </v-flex>
        ...
        <v-flex xs12 sm12 md6  order-xs1  order-md2 order-lg2 order-xl2>
            <v-layout row wrap >
                  ...
                  <v-flex xs12 sm12 md12 elevation-1  style="min-height:440px;">
                    <Charts :title="'最近30分钟异常数量2'" :option="last30Option" :theme="'light'" ref="last30Chart"/>
                  </v-flex>
...
import ChartOptioner from '@/$base/utils/ChartOptioner'
...
export default {
  ...
  created: function () {
    this.barOption = this.convertXBarOption()
    this.barOption2 = this.convertXBarOption2()
    this.graphOption = this.convertGraphOption() //  OptionUtil.genGraphOption()

    // this.getTrendByDate(types[0].name, types[0].uri, 5000, types[0].chartName) // 最近30分钟 5秒刷新一次
    this.last30Option = this.genLastOption()
    this.lastMonthOption = this.genPreMonth()
    this.lastWeekOption = this.genPreWeek()
    this.lastDayOption = this.genPreWeek()
  ...
  methods: {
    ...
    // 生成X轴显示数据, Y轴是categorybar图。
    convertXBarOption () {
      const uri = 'http://172.30.49.69:8080/welab-skyeye/v1/top/{dimension}'
      const params = {
      ...
      const res = Mock.lastTop10
      const category = res.x
      const value = res.y
      const bgValue = res.y.map(item => item + 200)
      const chartOptioner = new ChartOptioner()
      return chartOptioner.convertBarOption('最近30分钟Top 10 App异常数量', category, value, bgValue, '数量')
    },
    convertXBarOption2 () {
      const res = Mock.lastTop10
      const category = res.x
      ...
      const bgValue = res.y.map(item => item + 200)
      const chartOptioner = new ChartOptioner()
      return chartOptioner.convertBarOption('最近1小时Top 10 App异常数量', category, value, bgValue, '数量')
    },

    // 生成依赖图
    convertGraphOption () {
      const list = Mock.graph
      const optioner = new ChartOptioner()
      const nodes = optioner.convertResult2Nodes(list, 'appName', 'num', 'app', [])
      const nodes2 = optioner.convertResult2Nodes(list, 'classNames', 'classNum', 'class', nodes)
      const links = optioner.convertResult2Links(list, 'appName', 'classNames')
      console.log(JSON.stringify(nodes.concat(nodes2)))
      const option = optioner.convertGraphOption('app和class依赖图', nodes.concat(nodes2), links, '数量', [{name:'app'}, {name: 'class'}])
      return option
    },
    ...
    genLastOption () {
      ...
      option.setXAxis(label)
        .setYAxis()
        .setLegend(['最大数量', '最小数量', '平均数量', '总数量'])
        .addSeries(max, 'line', '最大数量', 0 , { key: 'lineStyle', value: dashLine} )
        .addSeries(min, 'line', '最小数量', 0)
        .addSeries(avg, 'line', '平均数量', 0)
        .addSeries(all, 'line', '总数量', 0)
      return option
    },
